<template name="approvaling">
	<view class="container">
		<view class="itemList">
			<mescroll-uni ref="mescrollRef" @init='mescrollInit' @down="downCallback" @up="upCallback" :up="upOption"  class="message" :top='220' :bottom='160'>
				<view class="itemBox"  v-for="(item,index) in dataList" :key="item.id">
					...
					<view class="itemInfo" @click="goDetail(item.id,item.billType,item.auditState)">
						<view class="info" >
							<view class="infoItem">
		...
		<view class="btnBox">
			<button type="primary" class="approveBtn" @click="allAudit">撤销提交</button>
...
<script>
	...
	export default{
		...
		data(){
			return{
				urlType:"/common/myBillPage?audit=2",//审批中
				dataList:[],
				idsArr:[],	//选中的bill的id			
				token:"",//token
				mescroll:null,//懒加载
		...
		methods:{
		   ...
		   allAudit: function(){
				...
				//没有选中任何单据
				if(idsArr.length==0&&idsArrKY.length==0){
				   uni.showToast({
				   	title:"请至少选择一项单据进行审批",
					icon:"none"
				   })
				...
				uni.showModal({
				    ...
				    success: function (res) {
				        if (res.confirm) {
						   ...
						   _this.$http.GET('/common/batchCancel',params)
						   .then(res=>{
								if(res.data.code==0){
									msg = "撤销提交成功"
									setTimeout(()=>{
										uni.redirectTo({
											url:"/pages/myBill/index?approvalingFlag=2"
										})
									},1500)
								}else{
									msg = "撤销提交失败"
