				...
				<view class="billMsgBox" v-for='(item,index) in pageData' :key="index">
					...
					<view class='contentright'>
						<view class="peojectList">
							<text class='left'>收支科目编码：</text>
							<text class='right'>{{item.code}}</text>
						</view>
						<view class="peojectList">
							<text class='left'>收支科目名称：</text>
							<text class='right' :title='item.content'>{{item.name}}</text>
						</view>
					</view>
				</view> 
			</mescroll-uni>
...
<script>
	...
	export default{
		props:['departRows', 'subjectIndex', 'budgetDepartid', 'billType'],
		data() {
			return {
		...
		methods:{
			...
			getData:function(code){
				...
				this.$http.GET("/datas/incomeexpendituresubject/page",params).then((res)=>{
					if(res.data.code==0){
			...
			upCallback:function(mescroll){
				...
				setTimeout(()=>{ 
					if(this.totalpage > this.currentpage){
						...
						this.$http.GET("/datas/incomeexpendituresubject/page",params)
						.then((res)=>{
							if(res.data.code==0){
			...
			radioClick:function(value){
				...
				let departIdtext=this.budgetDepartid; //预算部门
				let backData = {};//回调函数
				//费用报销单
				if (this.billType == "FY") {
					backData={
						subjectIndex: this.subjectIndex,//索引
						payprojectcode:checkData.code,//支出项目编码
						payprojectName:checkData.name,//支出项目名称
						budgetSubjectCode:checkData.budgetSubjectCode, //预算科目编码
						budgetSubjectName:checkData.budgetSubjectName, //预算科目名称
						budgetCentralizDepartname:checkData.departName, //预算归口部门
						budgetCentralizDepartid:checkData.departId, //预算归口部门id
						relationBudget:checkData.relationBudget, //是否关联预算
					}
					//是否车辆/船舶报销(1:否)
					if(checkData.shipOrCar=='1'){
						backData.isShow = '1';
						backData.licensePlate = '';
					}else{
						backData.isShow = '0';
					}
					//支出项目带出预算信息
					this.$http.GET(`/budget/budgetImplement/getBudgetAmount`, {
						params: {
						  code: checkData.budgetSubjectCode, //预算编码
						  departId:departIdtext, //预算部门
						}
					}).then((res)=> {
						if (res.data.code == 0) {
							if(res.data.code==0){
								backData.budgetAmountSum = res.data.data.budgetAmountSum;//预算总额
								backData.budgetExecutedAmount = res.data.data.budgetExecutedAmount;//预算已执行额
								backData.budgetSurplusAmount = res.data.data.budgetSurplusAmount;//预算剩余额度
								backData.percentage = res.data.data.percentage;//预算剩余额度
							}
						}
