<template name="approvaling">
	<view class="container">
		<view class="itemList">
			<mescroll-uni ref="mescrollRef" @init='mescrollInit' @down="downCallback" @up="upCallback" :up="upOption"  class="message" :top='220' :bottom='160'>
				<view class="itemBox"  v-for="(item,index) in dataList" :key="item.id">
					...
					<view class="itemInfo" @click="goDetail(item.id,item.billType,item.audit)">
						<view class="info" >
							<view class="infoItem">
						...
						<view class="moneyBox">
							<view class="pedding" v-if="item.audit == 0">
								待提交
							</view>
							<view class="pedding" v-if="item.audit == -2">
								暂存
							</view>
							<view class="price">
								￥{{item.amount}}
		...
		<view class="btnBox">
			<button type="primary" class="approveBtn" @click="allAudit">批量提交</button>
		</view>
	</view>
</template>
...
<script>
	...
	export default{
		...
		data(){
			return{
				urlType:"/common/myBillPage?flag=2",//待提交=保存+暂存单据
				dataList:[],
				idsArr:[],	//选中的bill的id			
				token:"",//token
				mescroll:null,//懒加载
		...
		methods:{
		   ...
		   allAudit: function(){
				...
				uni.showModal({
				    ...
				    success: function (res) {
				        if (res.confirm) {
						   ...
						   _this.$http.GET('/common/batchCommit',params)
						   .then(res=>{
								if(res.data.code==0){
									msg = "提交成功"
									setTimeout(()=>{
			...
			//查看详情
			goDetail:function (id,billType,auditState){
				if(billType=="FY"){
					uni.navigateTo({
						url:"/pages/reim/costExpense/costExpense?id="+id+"&billType="+billType+"&auditState="+auditState+"&isWrite=true"
					})	
				} else if(billType=="QK"){
					uni.navigateTo({
						url:"/pages/reim/reimRequiser/reimRequiser?id="+id+"&billType="+billType+"&auditState="+auditState+"&isWrite=true"
					})	
				} else if(billType=="SQSQ"){
					uni.navigateTo({
						url:"/pages/reim/priorApproval/priorApproval?id="+id+"&billType="+billType+"&auditState="+auditState+"&isWrite=true"
					})	
				} else if(billType=="GWJD"){ //公务接待
					uni.navigateTo({
						url:"/pages/reim/receptionApply/receptionApply?id="+id+"&billType="+billType+"&auditState="+auditState+"&isWrite=true"
					})	
				} else if(billType=="YWBX"){ //公务报销
					uni.navigateTo({
						url:"/pages/reim/receptionExpense/receptionExpense?id="+id+"&billType="+billType+"&auditState="+auditState+"&isWrite=true"
					})	
				} else if(billType=="CC"){ //出差申请
					uni.navigateTo({
						url:"/pages/reim/travelApply/travelApply?id="+id+"&billType="+billType+"&auditState="+auditState+"&isWrite=true"
					})	
				} else if(billType=="CLBX"){ //出差报销
					uni.navigateTo({
						url:"/pages/reim/travelExpense/travelExpense?id="+id+"&billType="+billType+"&auditState="+auditState+"&isWrite=true"
					})	
				} else{
					uni.showToast({
...
<style scoped>
	...
	.approveBtn{margin-top: 0px;width: 100%;width: 90%;background-color: #599af6;}
